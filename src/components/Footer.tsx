...
const Footer = () => {
  ...
  const whatsappNumber = settings.whatsappNumber || "5511934529229";
    ...
    <footer className="py-16 border-t border-border bg-background">
      <div className="container px-6">
        <div className="grid md:grid-cols-3 gap-10 mb-10">
          <div>
            <img src={logo} alt="ProtechX" className="h-12 object-contain mb-4" />
            <p className="text-sm text-muted-foreground leading-relaxed">{c.tagline}</p>
          </div>
          <div>
            <h4 className="font-display text-sm font-bold uppercase tracking-wider text-foreground mb-4">Navegação</h4>
          ...
          <div>
            <h4 className="font-display text-sm font-bold uppercase tracking-wider text-foreground mb-4">Contato</h4>
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li><a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">WhatsApp</a></li>
        ...
        <div className="border-t border-border pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted-foreground">© {new Date().getFullYear()} ProtechX Soluções Inteligentes. Todos os direitos reservados.</p>
